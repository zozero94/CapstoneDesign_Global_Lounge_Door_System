package capstonedesign.globallounge.model.permission

import Encryption.Encryption
import capstonedesign.globallounge.dto.User
import capstonedesign.globallounge.mainjob.MainPresenter
...
object ServerConnection : BaseServer() {
    ...
    /**
     * 서버로 사용자 상태 on을 요청
     * @see capstonedesign.globallounge.qrjob.QrPresenter.stateRequest
     */
    fun sendStateOn() {
        with(JsonObject()) {
    ...
    /**
     * 서버로 사용자 상태 off를 요청
     * @see capstonedesign.globallounge.qrjob.QrPresenter.stateDelete
     */
    fun sendStateOff() {
        with(JsonObject()) {
    ...
    /**
     * 사용자 로그아웃을 요청
     * @see capstonedesign.globallounge.qrjob.QrPresenter.logout
     */
    fun logout() {
        with(JsonObject()) {
    ...
    /**
     * 비콘범위 들어와서 ShakeAlgorithm 실행시 호출
     * @see capstonedesign.globallounge.model.beacon
     */
    fun openAdmin() {
        with(JsonObject()) {
